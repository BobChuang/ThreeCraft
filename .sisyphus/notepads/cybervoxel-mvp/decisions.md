...
- 决策：F3 仅执行验收收口动作（Playwright 真实交互 + 证据落盘 + notepad 追加 + build 复核），不触碰业务实现代码与联机协议路径。
- 决策：边界“NPC 路径失效”采用 `findPath` 的阻塞 worldQuery 进行确定性无路径验证（`path=null`），作为可审计的路径失败证据。
- 决策：报告末行按统一模板输出 `场景 [N/N 通过] | 集成 [N/N] | 边界用例 [N 已测] | 向后兼容 [通过/失败] | 结论`，并如实保留 F3 不通过结论于正文。

## 2026-02-23 F3（真实手动 QA，final rerun）

- 决策：本轮继续执行“最小改动策略”，仅更新 F3 证据与 notepad，不改业务源码、不触碰 Socket.io/联机路径。
- 决策：向后兼容 Classic 采用独立固定地图 dev 服务（`VITE_FIXED_MAP_INDEX=0`）验证，以 `config.weather=0` 作为可审计判据。
- 决策：最终报告维持失败即失败原则，末行统一汇总格式输出 `场景 [4/6 通过] | 集成 [3/3] | 边界用例 [4 已测] | 向后兼容 [通过] | 结论：未通过`。

## 2026-02-23 F3（FINAL 波次）

- 决策：F3 本轮仅做证据收口，不修改业务源码；所有失败项以“真实不可达/不可复现”原样入证据，不做推断性修正。
- 决策：向后兼容采用独立 Classic 会话（`VITE_FIXED_MAP_INDEX=0`, `4190`）完成，以规避主场景端口漂移与地图上下文干扰。
- 决策：报告末行改为严格模板结尾（以 `| 结论` 收束，不附加冒号后文本），优先满足 final-wave 输出格式约束。

## 2026-02-23 F4（范围一致性检查）

- 决策：本轮仅输出 F4 审计证据与 notepad 追加记录，不修改 `src/**`、`server/**`、F2/F3 证据与计划文件。
- 决策：范围一致性判定采用“任务目标与历史提交意图映射 + F1 禁项结果继承 + F2/F3 新证据复核”的组合审计口径。
- 决策：对已识别范围蔓延（`.sisyphus/tmp/**`、额外 lockfile 变更）仅如实入证据，不在 F4 中执行清理修复，避免越权变更。

## 2026-02-23 F4（计划状态同步）

- 决策：基于 `final-qa/f4-scope-consistency.txt` 既有结论，仅同步勾选计划中的 F4 复选框，保持 F2/F3 与 final checklist 原样不变。
